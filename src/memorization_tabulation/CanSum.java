...
public class CanSum {
	...
	public static void main(String[] args) {
		...
		System.out.println(canSumTab(7, new int[] {5,3,4,7})); //true 
		System.out.println(canSumTab(7, new int[] {2,4})); //false 
		System.out.println(canSumTab(300, new int[] {7,14})); //false 
 ...
 You may assume that all input numbers are nonnegative. 
	...
	//space complexity O(m)
	public static boolean canSumTab(int targetSum, int[] nums) {
		...
		for(int i=0; i<=targetSum; i++) {
			if(table[i] == true) {
				for(int j=0; j<nums.length; j++) {
					int k = i+nums[j];
					if (k < table.length) table[k] = true;
				}
			}
		}
